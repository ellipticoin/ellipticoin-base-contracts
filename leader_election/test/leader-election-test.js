...
describe("LeaderElection", function() {
  ...
  beforeEach(async () => {
    ...
    await blockchain.loadFile("../target/wasm32-unknown-unknown/release/leader_election.wasm");
  });
  ...
  describe("submit_block", function() {
    it.only("set the latest block", async function() {
      await blockchain.call("constructor", randomSeed);
      let blockHash = sha3("");
      ...
      await blockchain.call("submit_block", blockHash, signature, new Buffer(PUBLIC_KEY, "hex"));
